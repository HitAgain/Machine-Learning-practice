...
def get_dis_matrix(data):
    ...
    """
    nPoint = len(data)
    dis_matrix = np.zeros((nPoint, nPoint))   ##二维numpy数组 n×n
...
def getW(data, k):
    ...
    for idx, each in enumerate(dis_matrix):
        index_array = np.argsort(each)   ##升序排列，返回排序后的index值
        W[idx][index_array[1:k+1]] = 1    ##取第idx行前近的 之所以从1开始 是因为每行对角元为零，所以最近的K个（不含自己）点从1开始计数
    tmp_W = np.transpose(W)      ###为了让最后的相似矩阵是对称的，因为i是j的前k近的点不必保证反过来也对 所以W加W转置除2保证实对称
    W = (tmp_W+W)/2
    return W
...
if __name__ == '__main__':
    ...
    L = getL(D, W)
    eigvec = getEigen(L)
    clf = KMeans(n_clusters=cluster_num)  ## 对laplace矩阵的特征向量 用K-means聚类
    s = clf.fit(eigvec)
    C = s.labels_
