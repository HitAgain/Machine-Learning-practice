...
# 找到每条数据距离哪个类中心最近    
def findClosestCentroids(X,initial_centroids):  #X&&initial_centroids is 2-dim nparray
    m = X.shape[0]                  # 数据条数
...
def computerCentroids(X,idx,K):
    ...
    for i in range(K):
        centroids[i,:] = np.mean(X[np.ravel(idx==i), :], axis=0).reshape(1,-1)   # trick是 np.ravle(idx == i)返回是否属于i类的bool类型，然后作为X的列表解析，选出所有属于i类的数据
    return centroids
