...
def PCA_face_example():
    '''加载数据并显示'''
    image_data = spio.loadmat('E:\机器学习主要算法Python实现\PCA\data_faces.mat')  
    X = image_data['X']
    ...
    '''拟合模型'''
    K=100
    model = pca.PCA(n_components=K).fit(x_train)   ###获得降维的模型 即特征值前k大对应的特征向量
    Z = model.transform(x_train)     ###做用在训练数据 降维
    Ureduce = model.components_      ##降维用的U矩阵 
    
    display_imageData(Ureduce[0:36,:])  #可视化部分U数据 看出主要提取了图片的主要特征和轮廓
    x_rec = np.dot(Z,Ureduce)           ##恢复
